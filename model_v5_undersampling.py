...
def plot_roc_pr_curves(test_metrics, out_dir, suffix=""):
    ...
    for t_idx, name in enumerate(names):
        ...
        plt.tight_layout()
        plt.savefig(f"{out_dir}/pr_{name}_dna(DDB setting){suffix}.png", dpi=200)
        plt.close()
...
def evaluate(model, loader, cfg):
    ...
    for batch in loader:
        ...
        for j, name in enumerate(names):
            ...
            if name in ['protein', 'rna', 'dna']:
                valid = valid & (disorder == 1)
    ...
    # ============================
    # ★ DeepDISOBind-Style Undersampling for RNA/DNA ★
    # applies ONLY to metrics that use y_true_all / y_prob_all
    # ============================
    TARGET_POS_RATE = 0.084  # 8.4%

    for task_j in [2, 3]:  # RNA=2, DNA=3
        if len(y_true_all[task_j]) == 0:
            continue

        y_true = np.concatenate(y_true_all[task_j], axis=0)
        y_prob = np.concatenate(y_prob_all[task_j], axis=0)
        ...
        desired_total = int(num_pos / TARGET_POS_RATE)
        desired_neg = max(1, desired_total - num_pos)

        desired_neg = min(desired_neg, num_neg)

        sampled_neg_idx = np.random.choice(neg_idx, size=desired_neg, replace=False)

        final_idx = np.concatenate([pos_idx, sampled_neg_idx])
        np.random.shuffle(final_idx)

        # overwrite the collected arrays (VERY IMPORTANT)
        y_true_all[task_j] = [y_true[final_idx]]
        y_prob_all[task_j] = [y_prob[final_idx]]
...
def main():
    ...
    for ep in range(1, cfg.max_epochs+1):
        ...
        rna_f1 = best_f1_per_task[3]
        if rna_f1 > best_rna_f1:
            best_rna_f1 = rna_f1
            best_metrics = test_metrics
            print(f"[BEST] Updated testRNA best-F1 = {best_rna_f1:.4f} at epoch {ep}")
