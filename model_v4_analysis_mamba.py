...
class MambaPython(nn.Module):
    ...
    def forward(self, x, return_update: bool = False):
        """
        x: (B,L,D)
        return_update=True면 (out, update_score) 반환
        update_score: (B,L)  각 residue에서 gate 평균값
        """
        B, L, D = x.shape
        ...
        # ★ Selective Update score (residue-wise)
        # gate가 클수록 "현재 입력을 더 반영" → 업데이트 강함
        update_score = g.mean(dim=-1)        # (B,L)
        ...
        out = self.norm(x + out)                     # (B,L,D)

        if return_update:
            return out, update_score
        return out
...
class IDRModel(nn.Module):
    ...
    def forward(self, emb, mask=None, return_update: bool = False):
        # emb: (B, L, d)
        real, imag = self.cwt(emb)  # (B, D_w, S, L)

        ctx_R, att_R = self.real_att(emb, real)  # (B, L, D_att), (B,L,S)
        ...
        H = E_fused
        update_layers = []  # (num_blocks, B, L)

        for blk in self.mamba_blocks:
            if return_update:
                H, upd = blk(H, return_update=True)  # upd: (B,L)
                update_layers.append(upd)
            else:
                H = blk(H)
        ...
        if return_update:
            # (nb, B, L) -> (B, L) 평균 update map
            update_layers = torch.stack(update_layers, dim=0)      # (nb,B,L)
            ret["update_layers"] = update_layers                  # 선택: layer별 분석용
            ret["update_map"] = update_layers.mean(dim=0)         # (B,L)
...
def plot_update_and_gt(
    protein_id: str,
    update_score: torch.Tensor,  # (Lmax,)
    gt: torch.Tensor,            # (Lmax,)
    mask: torch.Tensor,          # (Lmax,) bool
    task_name: str,
    out_dir: str
):
    """
    ...
    upd = update_score[:L].detach().cpu().numpy()
    gt_ = gt[:L].detach().cpu().numpy()
    x = np.arange(L)

    fig, ax1 = plt.subplots(figsize=(12, 3))

    # update score line
    ax1.plot(x, upd, linewidth=1.5)
    ax1.set_xlabel("Sequence position")
    ax1.set_ylabel("Selective update score")

    # GT track (binary)
    ax2 = ax1.twinx()
    ax2.fill_between(x, 0, gt_, step="mid", alpha=0.25)
    ax2.set_ylim(0, 1.2)
    ax2.set_yticks([0, 1])
    ax2.set_ylabel("GT binding")

    ax1.set_title(f"{protein_id} | {task_name}")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"{protein_id}_{task_name}.png"), dpi=200)
    plt.close()



@torch.no_grad()
def save_update_maps_for_testset(model, loader, cfg, task_name: str):
    """
    test loader의 모든 샘플에 대해:
    - update_map (B,L)
    - GT track (B,L)
    를 한 장씩 저장한다.
    """
    ...
    task_to_idx = {"disorder": 0, "protein": 1, "rna": 2, "dna": 3}
    j = task_to_idx[task_name]

    out_dir = os.path.join(cfg.out_dir, f"update_map_cases_{task_name}")
    os.makedirs(out_dir, exist_ok=True)

    for batch in loader:
        H = batch["emb"].to(cfg.device)      # (B,L,D)
        Y = batch["label"].to(cfg.device)    # (B,L,4)
        M = batch["mask"].to(cfg.device)     # (B,L)
        ids = batch["ids"]                   # list[str]

        outputs = model(H, M, return_update=True)
        update_map = outputs["update_map"]   # (B,L)

        for b in range(H.shape[0]):
            plot_update_and_gt(
                protein_id=ids[b],
                update_score=update_map[b],
                gt=Y[b, :, j],
                mask=M[b],
                task_name=task_name,
                out_dir=out_dir
    ...
    print(f"[DONE] Saved per-sample figures to: {out_dir}")


@torch.no_grad()
def analyze_update_maps_task_specific(
    model,
    loader,
    cfg,
    task_name: str,
    ...
    """
    task_name에 해당하는 best model + GT만 사용해서
    update map 시각화를 수행
    """
    model.eval()
    ...
    for batch in loader:
        ...
        for b in range(H.size(0)):
            gt_task = Y[b, :, task_idx]      # (L,)
            mask = M[b]

            # 🔑 핵심 필터링 조건
            if only_positive:
                if (gt_task[mask] == 1).sum() == 0:
                    continue  # 이 task에 positive residue 없는 샘플은 스킵
...
def main():
    ...
    for task in tasks:
        ...
        analyze_update_maps_task_specific(
            model=model,
            loader=test_loader,
            cfg=cfg,
            task_name=task,
            only_positive=True   # ★ 핵심 옵션
        )
