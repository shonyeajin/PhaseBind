...
class ComplexInteractionBlock(nn.Module):
    ...
    def forward(self, R, I, return_components: bool=False):
        # R, I: (B, L, D)
        R_ = self.a * R - self.b * I
        I_ = self.b * R + self.a * I
        out = torch.cat([R_, I_], dim=-1)  # (B, L, 2D)
        fused = self.fuse(out)            # (B, L, D)
...
class IDRModel(nn.Module):
    ...
    def forward(self, emb, mask=None, return_cib: bool=False):
        # emb: (B, L, d)
        ...
        if return_cib:
            fused, R_post, I_post = self.cib(ctx_R, ctx_I, return_components=True)
        else:
        ...
        ret = {"logits": out, "att_R": att_R, "att_I": att_I}
        if return_cib:
            # pre-CIB streams
            ret["cib_R_pre"] = ctx_R
            ret["cib_I_pre"] = ctx_I
            # post-mixing streams (before concat+linear fuse)
            ret["cib_R_post"] = R_post
            ret["cib_I_post"] = I_post
        return ret
...
def evaluate(model, loader, cfg):
    ...
    cib_stats = {t: {
        ...
        "dlog_rho": [], "dalign": [], "dphase": [], "abs_dphase": []
    } for t in names}
    ...
    for batch in loader:
        ...
        outputs = model(H, M, return_cib=True)                # dict: name -> (B,L)
        ...
        for j, name in enumerate(names):
            ...
            if pos_mask.any():
                ...
                # ---- NEW: CIB pre/post tensors ----
                R_pre = outputs["cib_R_pre"][pos_mask]     # (N_pos, D_att)
                I_pre = outputs["cib_I_pre"][pos_mask]
                R_post = outputs["cib_R_post"][pos_mask]   # (N_pos, D_att)
                I_post = outputs["cib_I_post"][pos_mask]

                # (1) rho = ||I|| / ||R||
                nR_pre = _safe_norm(R_pre)
                nI_pre = _safe_norm(I_pre)
                nR_post = _safe_norm(R_post)
                nI_post = _safe_norm(I_post)

                rho_pre = (nI_pre / (nR_pre + 1e-12))
                rho_post = (nI_post / (nR_post + 1e-12))

                # 안정적으로는 log-ratio가 더 좋음
                dlog_rho = torch.log(rho_post + 1e-12) - torch.log(rho_pre + 1e-12)

                # (2) align = cos(R, I)
                align_pre = F.cosine_similarity(R_pre, I_pre, dim=-1, eps=1e-12)
                align_post = F.cosine_similarity(R_post, I_post, dim=-1, eps=1e-12)
                dalign = align_post - align_pre

                # (3) "representation phase"
                # 채널별 phase = atan2(I, R), 그리고 residue 단위로 circular mean
                phase_pre_ch = torch.atan2(I_pre, R_pre)     # (N_pos, D_att)
                phase_post_ch = torch.atan2(I_post, R_post)

                # circular mean over channels
                sin_pre = torch.mean(torch.sin(phase_pre_ch), dim=-1)
                cos_pre = torch.mean(torch.cos(phase_pre_ch), dim=-1)
                phase_pre = torch.atan2(sin_pre, cos_pre)   # (N_pos,)

                sin_post = torch.mean(torch.sin(phase_post_ch), dim=-1)
                cos_post = torch.mean(torch.cos(phase_post_ch), dim=-1)
                phase_post = torch.atan2(sin_post, cos_post)

                dphase = _circular_diff(phase_post, phase_pre)
                abs_dphase = torch.abs(dphase)

                # save (cpu numpy)
                cib_stats[name]["rho_pre"].append(rho_pre.detach().cpu().numpy())
                cib_stats[name]["rho_post"].append(rho_post.detach().cpu().numpy())
                cib_stats[name]["dlog_rho"].append(dlog_rho.detach().cpu().numpy())

                cib_stats[name]["align_pre"].append(align_pre.detach().cpu().numpy())
                cib_stats[name]["align_post"].append(align_post.detach().cpu().numpy())
                cib_stats[name]["dalign"].append(dalign.detach().cpu().numpy())

                cib_stats[name]["phase_pre"].append(phase_pre.detach().cpu().numpy())
                cib_stats[name]["phase_post"].append(phase_post.detach().cpu().numpy())
                cib_stats[name]["dphase"].append(dphase.detach().cpu().numpy())

                cib_stats[name]["abs_dphase"].append(abs_dphase.detach().cpu().numpy())
    ...
    for t in names:
        cib_final[t] = {}
        for k, vlist in cib_stats[t].items():
            if len(vlist) == 0:
                cib_final[t][k] = np.empty((0,), dtype=np.float32)
            else:
                cib_final[t][k] = np.concatenate(vlist, axis=0)
...
def main():
    ...
    for task in tasks:
        ...
        cib = metrics["cib_analysis"][task]
        if cib["dlog_rho"].size > 0:
            print(f"[CIB] {task}: mean dlog_rho={cib['dlog_rho'].mean():.8f}, "
                  f"mean dalign={cib['dalign'].mean():.8f}, "
                  f"mean dphase={cib['dphase'].mean():.8f}")
            cib = metrics["cib_analysis"][task]
            for k in ["rho_pre","rho_post","align_pre","align_post"]:
                arr = cib[k]
                print(task, k, np.mean(arr), np.percentile(arr,[5,50,95]))
            arr = cib["abs_dphase"]
            print(
                task,
                "abs_dphase | mean:", arr.mean(),
                "p95:", np.percentile(arr, [5,50,95])
            )
        else:
            print(f"[CIB] {task}: no positive residues -> skip")
